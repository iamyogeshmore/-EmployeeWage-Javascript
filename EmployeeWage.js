...
while (
  totalWorkingDays < MAX_DAYS_IN_MONTH &&
...
let totalWages = empDailyHoursWageArray
  ...
  .reduce(
    (totalWages, dailyHoursAndWage) =>
      (totalWages += dailyHoursAndWage.dailyWage),
    0
  );
...
let totalHours = empDailyHoursWageArray
  ...
  .reduce(
    (totalHours, dailyHoursAndWage) =>
      (totalHours += dailyHoursAndWage.dailyHours),
    0
  );
...
empDailyHoursWageArray
  .filter((dailyHoursAndWage) => dailyHoursAndWage.dailyHours == 8)
  .forEach((dailyHoursAndWage) =>
    process.stdout.write(dailyHoursAndWage.toString())
  );
console.log(
  "\n******************************************************************"
);
